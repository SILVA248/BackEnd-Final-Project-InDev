...
router.get("/", authenticateToken, async (_req, res) => {
  ...
  const promise2 = asyncCon.query("SELECT application_id, first_name, last_name, email, phone, linkedinURL, salaryExp from applications LIMIT ?, ?", [offset, parseInt(perPage)]);
...
router.post("/", (_req, res) =>{
  ...
  const { first_name, last_name, email, phone, linkedinURL, salaryExp} = _req.body;

  const sql = `INSERT INTO applications
              (first_name, last_name, email, phone, linkedinURL, salaryExp
              VALUES (?,?,?,?,?,?)`;
  con.query(sql, [first_name, last_name, email, phone, linkedinURL, salaryExp], (e,r)   => {
    if (e) throw (e);
